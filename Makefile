...
geo: geo/ch.geojson \
	geo/ch-country.geojson  \
	geo/ch-districts.geojson \
	geo/ch-lakes.geojson \
	geo/ch-municipalities.geojson \
	geo/ch-plz.geojson \
	$(addprefix geo/muni/,$(addsuffix -municipalities.geojson,$(CANTONS))) \
...
clean:
	rm -rf build topo svg geo

.PHONY: clean clobber topo
...
geo/muni/%-municipalities.geojson: topo/%-municipalities.json
	mkdir -p $(dir $@)
	node_modules/topojson-client/bin/topo2geo \
		municipalities=$@ < $<
...
geo/ch-plz.geojson: topo/ch-plz.json
	mkdir -p $(dir $@)
	node_modules/topojson-client/bin/topo2geo \
		plz=$@ < $<	
